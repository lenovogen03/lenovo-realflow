...
function applyBrandingStyles(branding) {
  ...
  try {
    ...
    if (raw) {
      ...
      // treat the absence of the key as "no preference set yet".
      if (parsed && parsed.themeKey) userThemeActive = true;
    }
  } catch (e) {
    /* ignore — fall through to applying server branding */
  }
  if (userThemeActive) {
    // Still apply non-color branding (logo URLs, app name, etc.)
    // by simply returning here; ThemeContext handles all colors.
    return;
  }
