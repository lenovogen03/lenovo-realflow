  ...
  return (
    <div className="space-y-4" data-testid="system-check-panel">
      ...
      {Object.entries(groups).map(([grp, checks]) => (
        <Card key={grp} className="bg-[var(--brand-card)] border-[var(--brand-border)]">
          ...
          <CardContent className="space-y-2">
            {checks.map((c, i) => (
              <div
                key={i}
                className="flex items-center justify-between bg-[var(--brand-card)] border border-[var(--brand-border)] rounded-md px-3 py-2"
